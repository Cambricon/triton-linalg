...
func.func @extract_element_from_arith_addi(%arg0: tensor<128xi32>, %arg1: tensor<128xi32>) -> i32 {
  ...
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]]] : tensor<128xi32>
  // CHECK-NEXT: %[[ADD:.*]] = arith.addi %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : i32
  // CHECK-NOT: linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_arith_addf(%arg0: tensor<128xf32>, %arg1: tensor<128xf32>) -> f32 {
  ...
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]]] : tensor<128xf32>
  // CHECK-NEXT: %[[ADD:.*]] = arith.addf %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : f32
  // CHECK-NOT: linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_arith_cmpi(%arg0: tensor<128xindex>, %arg1: tensor<128xindex>) -> i1 {
  // CHECK: %[[C0_INDEX:.*]] = arith.constant 0 : index
  // CHECK-NEXT: %[[ARG_EXTRA0:.*]] = tensor.extract %arg0[%[[C0_INDEX]]] : tensor<128xindex>
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]]] : tensor<128xindex>
  // CHECK-NEXT: %[[CMPI:.*]] = arith.cmpi slt, %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : index
  // CHECK-NOT: linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_arith_cmpf(%arg0: tensor<128xf32>, %arg1: tensor<128xf32>) -> i1 {
  ...
  // CHECK-NEXT: %[[ARG_EXTRA0:.*]] = tensor.extract %arg0[%[[C0_INDEX]]] : tensor<128xf32>
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]]] : tensor<128xf32>
  // CHECK-NEXT: %[[CMPF:.*]] = arith.cmpf olt, %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : f32
  // CHECK-NOT: linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_arith_and(%arg0: tensor<128xi32>, %arg1: tensor<128xi32>) -> i32 {
  ...
  // CHECK-NEXT: %[[ARG_EXTRA0:.*]] = tensor.extract %arg0[%[[C0_INDEX]]] : tensor<128xi32>
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]]] : tensor<128xi32>
  // CHECK-NEXT: %[[AND:.*]] = arith.andi %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : i32
  // CHECK-NOT: linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_map_arith_add_with_two_indices(
  ...
  // CHECK:      %[[C0_INDEX:.*]] = arith.constant 0 : index
  // CHECK-NEXT: %[[ARG_EXTRA0:.*]] = tensor.extract %arg0[%[[C0_INDEX]], %[[C0_INDEX]]] : tensor<128x16xi32>
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[%[[C0_INDEX]], %[[C0_INDEX]]] : tensor<128x16xi32>
  // CHECK-NEXT: %[[ADD:.*]] = arith.addi %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : i32
  // CHECK-NOT:  linalg.*
  %c0 = arith.constant 0 : index
...
func.func @extract_element_from_map_with_two_payloads(%arg0: tensor<32xi64>, %arg1: tensor<32xi32>) -> i64 {
  ...
  // CHECK: %[[EXT:.*]] = arith.extsi %[[ARG_EXTRA1]] : i32 to i64
  // CHECK: %[[ADD_0:.*]] = arith.addi %[[ARG_EXTRA0]], %[[EXT]] : i64
...
func.func @extract_element_from_expand_shape_op_with_src_dimension_size_1(%arg0: tensor<2x12xi32>) -> i32 {
  ...
  %0 = tensor.expand_shape %arg0 [[0], [1, 2]] : tensor<2x12xi32> into tensor<2x3x4xi32>
...
func.func @extract_element_from_expand_shape_op_with_static_shape(%arg0: tensor<128x64xi32>, %arg1: index) -> i32 {
  ...
  %0 = tensor.expand_shape %arg0 [[0, 1], [2, 3]] : tensor<128x64xi32> into tensor<4x32x4x16xi32>
...
func.func @extract_element_from_expand_shape_op_with_dynamic_shape(%arg0: tensor<?x?xi32>) -> i32 {
  ...
  %c2 = arith.constant 2 : index
  %c3 = arith.constant 3 : index
  %0 = tensor.expand_shape %arg0 [[0, 1], [2, 3]] : tensor<?x?xi32> into tensor<?x10x6x?xi32>
...
func.func @extract_from_for_iter_args(%arg0: i64, %arg1: tensor<64x64xf32>, %arg2: tensor<64x64xi64>, %arg3: tensor<64x64xi64>) -> tensor<64x64xf32> {
  ...
  %0:2 = scf.for %arg4 = %c0 to %c2 step %c1 iter_args(%arg5 = %arg1, %arg6 = %arg2) -> (tensor<64x64xf32>, tensor<64x64xi64>) {
    ...
    %mapped_0 = linalg.map { arith.addi } ins(%arg6, %arg3 : tensor<64x64xi64>, tensor<64x64xi64>) outs(%5 : tensor<64x64xi64>)
...
// CHECK-DAG:   %[[VAL0:.*]] = tensor.extract %[[ARG2:.*]][%[[C0]], %[[C0]]] : tensor<64x64xi64>
// CHECK:       %[[VAL1:.*]]:2 = scf.for %[[ARG4:.*]] = %[[C0]] to %[[C2]] step %[[C1]] iter_args(%[[ARG5:.*]] = %[[ARG1:.*]], %[[ARG6:.*]] = %[[VAL0]]) -> (tensor<64x64xf32>, i64) {
// CHECK:         %[[VAL2:.*]] = arith.index_cast %[[ARG6]] : i64 to index
// CHECK:         %[[PTR:.*]] = llvm.inttoptr %[[ARG0:.*]] : i64 to !llvm.ptr<1>
...
// CHECK:         %[[VAL5:.*]] = tensor.extract %[[ARG3:.*]][%[[C0]], %[[C0]]] : tensor<64x64xi64>
// CHECK:         %[[VAL6:.*]] = arith.addi %[[ARG6]], %[[VAL5]] : i64
// CHECK:         scf.yield %[[VAL_MAP]], %[[VAL6]] : tensor<64x64xf32>, i64
// CHECK:       }
...
func.func @extract_from_for_iter_args_fail_case2(%arg0: i64, %arg1: tensor<64x64xf32>, %arg2: tensor<64x64xi64>, %arg3: tensor<64x64xi64>) -> tensor<64x64xf32> {
  ...
  %0:2 = scf.for %arg4 = %c0 to %c2 step %c1 iter_args(%arg5 = %arg1, %arg6 = %arg2) -> (tensor<64x64xf32>, tensor<64x64xi64>) {
    ...
    %mapped_1 = linalg.map { arith.addi } ins(%mapped_0, %6 : tensor<64x64xi64>, tensor<64x64xi64>) outs(%7 : tensor<64x64xi64>)
...
// CHECK:           %[[VAL_2:.*]] = tensor.empty() : tensor<64xf32>
// CHECK:           %[[VAL_3:.*]] = linalg.map { math.absf } ins(%[[VAL_0]] : tensor<64xf32>) outs(%[[VAL_2]] : tensor<64xf32>)
// CHECK:           %[[VAL_4:.*]] = linalg.map { math.atan } ins(%[[VAL_0]] : tensor<64xf32>) outs(%[[VAL_3]] : tensor<64xf32>)
// CHECK:           %[[VAL_5:.*]] = tensor.extract %[[VAL_0]]{{\[}}%[[VAL_1]]] : tensor<64xf32>
// CHECK:           %[[VAL_6:.*]] = math.absf %[[VAL_5]] : f32
// CHECK:           %[[VAL_7:.*]] = math.exp %[[VAL_6]] : f32
// CHECK:           return %[[VAL_4]], %[[VAL_7]] : tensor<64xf32>, f32
// CHECK:         }
...
// CHECK:           %[[VAL_2:.*]] = tensor.empty() : tensor<64xf32>
// CHECK:           %[[VAL_3:.*]] = linalg.map { math.absf } ins(%[[VAL_0]] : tensor<64xf32>) outs(%[[VAL_2]] : tensor<64xf32>)
// CHECK:           %[[VAL_4:.*]] = linalg.map { math.exp } ins(%[[VAL_3]] : tensor<64xf32>) outs(%[[VAL_3]] : tensor<64xf32>)
// CHECK:           %[[VAL_5:.*]] = tensor.extract %[[VAL_0]]{{\[}}%[[VAL_1]]] : tensor<64xf32>
// CHECK:           %[[VAL_6:.*]] = math.atan %[[VAL_5]] : f32
// CHECK:           return %[[VAL_4]], %[[VAL_6]] : tensor<64xf32>, f32
// CHECK:         }
...
func.func @arith_addi_0d(%arg0: tensor<i32>, %arg1: tensor<i32>) -> i32 {
  // CHECK-NEXT: %[[ARG_EXTRA0:.*]] = tensor.extract %[[ARG0:.*]][] : tensor<i32>
  // CHECK-NEXT: %[[ARG_EXTRA1:.*]] = tensor.extract %[[ARG1:.*]][] : tensor<i32>
  // CHECK-NEXT: %[[ADD:.*]] = arith.addi %[[ARG_EXTRA0]], %[[ARG_EXTRA1]] : i32
  // CHECK-NOT: linalg.*
...
func.func @map_0d(%arg0: tensor<i32>, %arg1: tensor<i32>) -> i32 {
  ...
  %1 = linalg.map { arith.addi } ins(%arg0, %arg1 : tensor<i32>, tensor<i32>) outs(%0 : tensor<i32>)
...
func.func @map_0d_with_two_payloads(%arg0: tensor<i64>, %arg1: tensor<i32>) -> i64 {
  ...
  %c1 = arith.constant 1 : i64
  // CHECK: %[[ARG_EXTRA0:.*]] = tensor.extract %arg0[] : tensor<i64>
  // CHECK: %[[ARG_EXTRA1:.*]] = tensor.extract %arg1[] : tensor<i32>
  // CHECK: %[[EXT:.*]] = arith.extsi %[[ARG_EXTRA1]] : i32 to i64
  // CHECK: %[[ADD_0:.*]] = arith.addi %[[ARG_EXTRA0]], %[[EXT]] : i64
  // CHECK: %[[ADD_1:.*]] = arith.addi %[[ADD_0]], %[[C1_I64]] : i64
  %0 = tensor.empty() : tensor<i64>
...
func.func @expand_shape_op_0d(%arg0: tensor<i32>) -> i32 {
  ...
  // CHECK-NOT: tensor.collapse_shape
  %c0 = arith.constant 0 : index
  %0 = tensor.expand_shape %arg0 [] : tensor<i32> into tensor<1x1xi32>
...
func.func @for_op_with_0d_iter_args(%arg0: i64, %arg1: tensor<f32>, %arg2: tensor<i64>, %arg3: tensor<i64>) -> tensor<f32> {
  ...
  %0:2 = scf.for %arg4 = %c0 to %c2 step %c1 iter_args(%arg5 = %arg1, %arg6 = %arg2) -> (tensor<f32>, tensor<i64>) {
    ...
    %mapped_0 = linalg.map { arith.addi } ins(%arg6, %arg3 : tensor<i64>, tensor<i64>) outs(%5 : tensor<i64>)
...
// CHECK-DAG:   %[[C1:.*]] = arith.constant 1 : index
// CHECK-DAG:   %[[C2:.*]] = arith.constant 2 : index
// CHECK-DAG:   %[[VAL0:.*]] = tensor.extract %[[ARG2:.*]][] : tensor<i64>
// CHECK:       %[[VAL1:.*]]:2 = scf.for %[[ARG4:.*]] = %[[C0]] to %[[C2]] step %[[C1]] iter_args(%[[ARG5:.*]] = %[[ARG1:.*]], %[[ARG6:.*]] = %[[VAL0]]) -> (tensor<f32>, i64) {
// CHECK:         %[[VAL2:.*]] = arith.index_cast %[[ARG6]] : i64 to index
// CHECK:         %[[PTR:.*]] = llvm.inttoptr %[[ARG0:.*]] : i64 to !llvm.ptr<1>
...
// CHECK:         %[[VAL5:.*]] = tensor.extract %[[ARG3:.*]][] : tensor<i64>
// CHECK:         %[[VAL6:.*]] = arith.addi %[[ARG6]], %[[VAL5]] : i64
// CHECK:         scf.yield %[[VAL_MAP]], %[[VAL6]] : tensor<f32>, i64
// CHECK:       }
