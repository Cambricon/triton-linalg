...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "How to style the 'lines' argument for the given binary. Can be set "
    ...
    if args.binary not in BINARY_TO_DEFAULT_REGEX:
        if not args.regex:
            raise parser.error(
                "If 'binary' is not 'yapf' or 'clang-format' then "
                "--regex must be set.")
    ...
    if args.lines_style not in ["yapf", "clang-format"]:
        raise parser.error(
            f"Unexpected value for --line-style {args.lines_style}")
