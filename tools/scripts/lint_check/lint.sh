...
files=`find . -name "*.py" |grep -v "./triton/" |grep -v "./lint_check"`
...
echo "***** check-header *****"
files=`find . \( -name "*.td" -o -name "*.h" \) -o -name "*cpp" |grep -v "./triton/"|grep -v "./lint_check"`
SKIP_FILE_LIST="./triton/ ./bin/RegisterTritonLinalgDialects.h"
SKIP_FILE_LIST=$(echo $SKIP_FILE_LIST | tr ' ' '\n')
for file in $files; do
  if echo "$SKIP_FILE_LIST" | grep -Fxq "$file"; then
    echo "***** skip $file for format_checker *****"
    continue
  fi
  python3 ${SCRIPTS_DIR}/format_checker.py --path $file
done
